...
def _variable_on_cpu(name, shape, initializer, use_fp16=False, trainable=True):
  ...
  with tf.device('/cpu:0'):
    ...
    var = tf.Variable(name=name, shape=shape, initial_value=init, dtype=dtype, trainable=trainable)
  return var
...
def conv2d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
      ...
      kernel = tf.cast(kernel, tf.float64)
...
def batch_norm_dist_template(inputs, is_training, scope, moments_dims, bn_decay):
  ...
  with tf.compat.v1.variable_scope(scope) as sc:
    ...
    ##cast the inputs into double
    inputs = tf.cast(inputs, tf.float64)

    def train_bn_op():
      batch_mean, batch_var = tf.nn.moments(inputs, moments_dims, name='moments')
      decay = bn_decay if bn_decay is not None else 0.9
      train_mean = tf.compat.v1.assign(pop_mean, tf.cast(pop_mean * decay, tf.float32) + tf.cast(batch_mean * (1 - decay), tf.float32), 
                    tf.float32)
      train_var = tf.compat.v1.assign(pop_var, tf.cast(pop_var * decay, tf.float32) + tf.cast(batch_var * (1 - decay), tf.float32), 
                    tf.float32)
      with tf.control_dependencies([train_mean, train_var]):
        print("batch_mean: {}".format(type(batch_mean)))
        print("batch_var: {}".format(type(batch_var)))
        print("beta: {}".format(type(beta)))
        print("gamma: {}".format(type(gamma)))
